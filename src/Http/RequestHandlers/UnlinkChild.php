...
use Jefferson49\Webtrees\Module\WebtreesApi\Helpers\RelationshipLinks;
...
final class UnlinkChild implements WebtreesMcpToolRequestHandlerInterface
{
    public const string METHOD_DESCRIPTION = 'Remove an individual from a family as a child, preserving both records.';
    public const string INDI_XREF_DESCRIPTION = 'The XREF of the child individual to unlink.';
    public const string FAM_XREF_DESCRIPTION = 'The XREF of the family from which the child shall be unlinked.';

    public function __construct(private TreeService $tree_service) {}

    #[OA\Delete(
        path: '/' . WebtreesApi::PATH_UNLINK_CHILD,
        tags: ['webtrees'],
        ...
        responses: [
            new OA\Response(response: '200', description: 'Successfully unlinked child.', content: new OA\MediaType(mediaType: 'application/json', schema: new OA\Schema(ref: XrefItem::class))),
    ...
    private function unlink(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $removed = RelationshipLinks::removeFactsTo($individual, ['FAMC'], $family);
    ...
    {
        return [
            'name' => WebtreesApi::PATH_UNLINK_CHILD,
            'description' => self::METHOD_DESCRIPTION,
            ...
            'annotations' => ['title' => WebtreesApi::PATH_UNLINK_CHILD, 'readOnlyHint' => false, 'destructiveHint' => true, 'idempotentHint' => true, 'openWorldHint' => true, 'deprecated' => false],
        ];
    }
}
